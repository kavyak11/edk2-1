  ...
  )
{
  ...
  Status                  =  GetUplUint64 (UPL_KEY_SERIAL_PORT_BAUDRATE, &BaudRate);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }

  Serial->BaudRate = (UINT32)BaudRate;
  Status           =  GetUplUint64 (UPL_KEY_SERIAL_PORT_REGISTER_BASE, &RegisterBase);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  ...
  Status =  GetUplUint64 (UPL_KEY_SERIAL_PORT_REGISTER_STRIDE, &RegisterStride);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  Status = GetUplUint64 (UPL_KEY_ACPI_TABLE_RSDP, &Rsdp);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  Status = GetUplUint64 (UPL_KEY_MEMORY_SPACE, &SizeOfMemorySpace);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "UPL data doesn't contain MemorySpace\n"));
    return EFI_NOT_FOUND;
  }

  Status = GetUplUint64 (UPL_KEY_IO_SPACE, &SizeOfIoSpace);
  if (EFI_ERROR (Status)) {
