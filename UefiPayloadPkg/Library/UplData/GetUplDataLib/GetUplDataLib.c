  ...
  )
{
  ...
  for (LocalIndex = 0; LocalIndex < *Count && LocalIndex < (Size -Index); LocalIndex++) {
    ...
    RETURN_ON_ERROR (CborDecoderGetUint64 (UPL_KEY_ATTRIBUTES, &Data[LocalIndex].Attributes, &SubMap));
  ...
  )
{
  ...
  for (LocalIndex = 0; LocalIndex < *Count && LocalIndex < (Size -Index); LocalIndex++) {
    ...
    RETURN_ON_ERROR (CborDecoderGetUint64 (UPL_KEY_ATTRIBUTES, (UINT64 *)&Data[LocalIndex].ResourceAttribute, &SubMap));
