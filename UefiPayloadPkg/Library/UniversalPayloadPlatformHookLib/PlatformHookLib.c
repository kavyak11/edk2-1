  ...
  )
{
  ...
  if (GenericHeader->Revision == PLD_SERIAL_PORT_INFO_REVISION) {
    SerialPortInfo = (PLD_SERIAL_PORT_INFO *) GET_GUID_HOB_DATA (GuidHob);
    if (GenericHeader->Length < PLD_SIZEOF_THROUGH_FIELD (PLD_SERIAL_PORT_INFO, RegisterBase)) {
      //
      // Retrun if can't find the Serial Port Info Hob with enough length
      //
      return EFI_NOT_FOUND;
    }
