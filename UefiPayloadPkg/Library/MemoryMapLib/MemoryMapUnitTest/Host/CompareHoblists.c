  ...
  )
{
  ...
  for (Index = 0; Index < MemoryMapHob->Count; Index++) {
    ...
    {
      if (ContainInMemMap) {
        DEBUG ((DEBUG_ERROR, "Point(%llx) already exists in another memory map descriptor\n", Point));
  ...
  )
{
  ...
  while (!END_OF_HOB_LIST (Hob)) {
    if (Hob.Header->HobType == EFI_HOB_TYPE_RESOURCE_DESCRIPTOR) {
         ...
         && (Point < (Hob.ResourceDescriptor->PhysicalStart + Hob.ResourceDescriptor->ResourceLength)))
      {
        if (ContainInResourceHob) {
          DEBUG ((DEBUG_ERROR, "Error: Point(%llx) already exist in more than one resource hob, hob[%d] and hob[%d] in hoblist%d\n", Point, ResourceHobIndex, Index, HoblistIndex));
        }
    ...
    if (Hob.Header->HobType == EFI_HOB_TYPE_MEMORY_ALLOCATION) {
      ...
      {
        if (ContainInMemHob) {
          DEBUG ((DEBUG_ERROR, "Error: Point(%llx) already exist in more than one memory hob, hob[%d] and hob[%d] in hoblist%d\n", Point, MemHobIndex, Index, HoblistIndex));
        }
    ...
    if (Hob.Header->HobType == EFI_HOB_TYPE_HANDOFF) {
      ...
      {
        if (ContainInHobList) {
          DEBUG ((DEBUG_ERROR, "Error: Point(%llx) already exist in more than one resource hob, hob[%d] and hob[%d] in ", Point, ResourceHobIndex, Index));
        }
  ...
  )
{
  ...
  AllArray = (UINT64 *)AllocatePool (sizeof (UINT64) * (AllHobCount * 7 + 12 * 2));
  if (AllArray == NULL) {
    DEBUG ((DEBUG_ERROR, "Memory cannot be allocated\n"));
    ASSERT (FALSE);
  }
  ...
  for (UINTN j = 0; j < AllHobCount * 7; j++) {
    ...
    if ((Info1.Attribute == Info2.Attribute) && (Info1.MemType == Info2.MemType) && (Info1.ResType == Info2.ResType) && (Info1.IsPhysicalMem == Info2.IsPhysicalMem)) {
      ...
      if ((Info1.Attribute == Info2.Attribute) && (Info1.MemType == Info2.MemType) && (Info1.IsPhysicalMem == Info2.IsPhysicalMem) && (Info2.MemType == EfiReservedMemoryType) && (Info2.ResType == EFI_RESOURCE_MEMORY_RESERVED)) {
        ...
        DEBUG ((DEBUG_ERROR, "The point which has issue is %llx\n", AllArray[j]));
        DEBUG ((DEBUG_ERROR, "Index in hob 1\n"));

        DEBUG ((DEBUG_ERROR, "Index of ResourceHobIndex is %d\n", Info1.ResourceHobIndex));
        DEBUG ((DEBUG_ERROR, "Index of MemHobIndex is %d\n", Info1.MemHobIndex));
        DEBUG ((DEBUG_ERROR, "Index of HobListIndex is %d\n", Info1.HobListIndex));

        DEBUG ((DEBUG_ERROR, "Index in hob 2\n"));
  ...
  )
{
  ...
  // For each resource hob or memory hob, pick 7 key point
  // For each phit hob, pick 12 key point
  AllArray = (UINT64 *)AllocatePool (sizeof (UINT64) * (AllCount * 7 + 12));
  if (AllArray == NULL) {
    DEBUG ((DEBUG_ERROR, "Memory cannot be allocated\n"));
    ASSERT (FALSE);
  }
